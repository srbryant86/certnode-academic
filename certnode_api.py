...
class CertNodeAPI:
    ...
    def _setup_routes(self):
        ...
        def before_request():
            ...
            if client_ip in self.rate_limits:
                recent_requests = [
                    t for t in self.rate_limits[client_ip] if t > cutoff_time
                ]
        ...
        self.app.add_url_rule(
            "/api/v1/certify", "certify", self.certify_content, methods=["POST"]
        )
        self.app.add_url_rule(
            "/api/v1/verify", "verify", self.verify_content, methods=["POST"]
        )
        self.app.add_url_rule(
            "/api/v1/verify/<ics_hash>",
            "verify_hash",
            self.verify_by_hash,
            methods=["GET"],
        )
        self.app.add_url_rule(
            "/api/v1/badge/<cert_id>", "badge", self.get_badge, methods=["GET"]
        )
        self.app.add_url_rule(
            "/api/v1/status", "status", self.get_status, methods=["GET"]
        )
        self.app.add_url_rule(
            "/api/v1/vault/stats", "vault_stats", self.get_vault_stats, methods=["GET"]
        )
        self.app.add_url_rule(
            "/api/v1/vault/search", "vault_search", self.search_vault, methods=["GET"]
        )
    ...
    def certify_content(self):
        ...
        try:
            ...
            if cert_type not in self.config.CERT_TYPES:
                raise BadRequest(
                    f"Invalid cert_type. Must be one of: {list(self.config.CERT_TYPES.keys())}"
                )
            ...
            if result.success and result.ics_signature:
                ...
                if not vault_stored:
                    self.logger.warning(
                        "Failed to store certification in vault",
                ...
                response_data.update(
                    {
                        ...
                        "verification_url": result.ics_signature.verification_data[
                            "verification_url"
                        ],
                        "badge_url": result.ics_signature.verification_data[
                            "badge_url"
                        ],
            ...
            self.logger.info(
                "Content certification completed via API",
                {
                    "success": result.success,
                    "cert_id": result.cert_id,
    ...
    def verify_content(self):
        ...
        try:
                        ...
                        jsonify(
                            {
                                "valid": False,
                                "errors": [
                                    f"No certification found for hash {data['ics_hash']}"
                                ],
                            }
                        ),
                        404,
            ...
            # Verify
            is_valid, errors = self.processor.verify_certification(
                content, signature_data
            )

            response_data = {"valid": is_valid, "errors": errors}
            ...
            if is_valid:
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Failed to parse signature for additional info: {str(e)}"
                    )

            self.logger.info(
                "Content verification completed via API",
                {"valid": is_valid, "client_ip": request.remote_addr},
            )
    ...
    def get_badge(self, cert_id: str):
        ...
        try:
            ...
            if hasattr(entry, "metadata") and "analysis_summary" in entry.metadata:
                ...
                badge_data.update(
                    {
                        "structural_score": analysis.get("frame_analysis", {}).get(
                            "structural_score"
                        ),
                        "convergence_achieved": analysis.get("cdp_analysis", {}).get(
            ...
            combined_status = {
                **status,
                "vault_stats": {
                    ...
                    "unresolved_drift_alerts": vault_stats.get(
                        "unresolved_drift_alerts", 0
                    ),
                },
                "api_info": {
                    "version": "1.0.0",
    ...
    def health_check(self):
        ...
        try:
            ...
            health_status = {
                ...
                "checks": {"processor": "ok", "vault": "ok", "ics_generator": "ok"},
            }
...
@app.post("/validate")
def validate(input: ValidationInput):
